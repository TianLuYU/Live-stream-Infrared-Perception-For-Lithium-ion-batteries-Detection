...
class SeqFormer(nn.Module):
    def __init__(self, detr, rel_coord=True, freeze_detr=False):
        ...
        self.controller_incoherent = MLP(hidden_dim, hidden_dim, self.num_gen_params_inc, 3)
        for contr in self.controller.layers:
            nn.init.xavier_uniform_(contr.weight)
            nn.init.zeros_(contr.bias)
    ...
    def forward(self, samples: NestedTensor, gt_targets, criterion, train=False):    
        ...
        for l, feat in enumerate(features[1:]):
            # src: [nf*N, _C, Hi, Wi], 5 512 40 96
            # mask: [nf*N, Hi, Wi], 5 1 40 96
            # pos: [nf*N, C, H_p, W_p]
            src, mask = feat.decompose() 
            src_proj_l = self.detr.input_proj[l](src)    # src_proj_l: [nf*N, C, Hi, Wi] 5 1 256 40 96
        ...
        query_embeds = None
        query_embeds = self.detr.query_embed.weight #300 512

        h_b, w_b = features[0].decompose()[0].shape[-2:]
        bottom_lvl_feature = features[0].decompose()[0].detach().clone().reshape(n // self.detr.num_frames,
                                                                                 self.detr.num_frames, c, h_b, w_b) #256

        c_r, h_b_r, w_b_r = feature_hr[0].decompose()[0].shape[-3:]
        bottom_lvl_feature_hr = feature_hr[0].decompose()[0].detach().reshape(n // self.detr.num_frames,
                                                                              self.detr.num_frames, c_r, h_b_r, w_b_r)
        ...
        for lvl in range(enc_lay_num):
            ...
            if lvl == enc_lay_num-1:
                dynamic_mask_head_params_inc = self.controller_incoherent(hs[lvl])
                dynamic_mask_head_params_refine = self.controller_refine(hs[lvl])
            ...
            mask_head_params_inc, mask_head_params_refine = [], []
            for i, indice in enumerate(indices):
                pred_i, tgt_j = indice
                num_insts.append(len(pred_i))
                mask_head_params.append(dynamic_mask_head_params[i, pred_i].unsqueeze(0))
                mask_head_params_inc.append(dynamic_mask_head_params_inc[i, pred_i].unsqueeze(0))
                mask_head_params_refine.append(dynamic_mask_head_params_refine[i, pred_i].unsqueeze(0))
            ...
            mask_head_params_inc = torch.cat(mask_head_params_inc, dim=1)
            mask_head_params_refine = torch.cat(mask_head_params_refine, dim=1)
 
            # mask prediction
            if lvl ==enc_lay_num-1:
                output_layers = self.forward_mask_head_train_with_inc(outputs_layer, memory, spatial_shapes,
                                                         reference_points, mask_head_params,
                                                         mask_head_params_inc, mask_head_params_refine,
                                                         num_insts, bottom_lvl_feature, bottom_lvl_feature_hr, img_feat,
                                                         outputs_coord.clone()[0].permute(1, 0, 2), False)
        ...
        outputs_class = torch.stack(outputs_classes) #6 5 300 2
        outputs_coord = torch.stack(outputs_coords) #6 5 1 300 4
        outputs_mask = outputs_masks
        # outputs_masks: dec_num x bs x [1, num_insts, 1, h, w]

        outputs['pred_logits'] = outputs_class[-1] #6里面的最后一个
        outputs['pred_boxes'] = outputs_coord[-1]
        outputs['pred_masks'] = outputs_mask[-1]
        outputs['pred_masks_inc'] = outputs_masks_inc[-1]
        outputs['pred_masks_refine'] = outputs_masks_refine[-1]
    ...
    def inference(self, samples: NestedTensor,orig_w,orig_h):
        ...
        feature_hr = features[-1:]
        # print("feature_hr", feature_hr[0].tensors.shape)
        feature_pos = pos[-1:]
        features = features[:-1]
        pos = pos[:-1]
        ...
        for lvl in range(enc_lay_num):
            ...
            sel_objs = list(set([indx // 2 for indx in indices10]))[:self.object_limit]
            ...
            if lvl == enc_lay_num -1:
                mask_head_params_inc = self.controller_incoherent(hs[lvl][:, sel_objs])    # [bs, num_quries, num_params]
                mask_head_params_refine = self.controller_refine(hs[lvl][:, sel_objs]) 
            # print("mask_head_params_inc", mask_head_params_inc.shape)
            # print(mask_head_params_refine.shape)
    ...
    def forward_mask_head_train_with_inc(self, outputs, feats, spatial_shapes, reference_points, mask_head_params,
                                         mask_head_params_inc, mask_head_params_refine, num_insts, bottom_lvl_feature,
                                         bottom_lvl_feature_hr, img_feat, pred_boxes, is_train):
        ...
        for iframe in range(self.detr.num_frames):
            ...
            fused_x = bottom_lvl_feature_fi + F.interpolate(fused_x, size=bottom_lvl_feature_fi.shape[-2:],
                                                            mode="nearest")
            ...
            ######### conv ##########
            mask_logits, mask_logits_c = self.dynamic_mask_with_coords(decod_feat_f, reference_points_i,
                                                                       mask_head_params,
                                                                       num_insts=num_insts,
                                                                       mask_feat_stride=8,
                                                                       rel_coord=self.rel_coord)
            mask_logits_inc, mask_logits_inc_c = self.dynamic_mask_with_coords_inc(fused_x, reference_points_i.detach(),
                                                                                   mask_head_params_inc, mask_logits_c,
                                                                                   num_insts=num_insts,
                                                                                   mask_feat_stride=4,
                                                                                   rel_coord=self.rel_coord)
            ...
            mask_logits_pred_inc_hr = (F.interpolate(mask_logits_inc_c.detach().sigmoid(),
                                                     size=bottom_lvl_feature_fi_hr.shape[-2:]) > 0.5).float()
            ...
            mask_logits_refine, _ = self.dynamic_mask_with_coords_inc_super(fused_x_hr, reference_points_i.detach(),
                                                                            mask_head_params_refine, mask_logits_inc_c,
                                                                            mask_logits_c, output_boxes,
                                                                            num_insts=num_insts,
                                                                            mask_feat_stride=2,
                                                                            rel_coord=self.rel_coord)
    ...
    def dynamic_mask_with_coords_inc_super(self, mask_feats, reference_points, mask_head_params, mask_logits_pred,
                                           mask_logits_pred_sub, output_boxes, num_insts,
                                           mask_feat_stride, rel_coord=True):
        # mask_feats: [N, C/32, H/8, W/8]
        # reference_points: [1, \sum{selected_insts}, 2]
        # mask_head_params: [1, \sum{selected_insts}, num_params]
        ...
        if rel_coord:
            ...
            for i, num_inst in enumerate(num_insts):
                ...
                mask_feats_bi_new = torch.cat((mask_feats_bi, mask_logits_pred_b, mask_logits_pred_b_sub,
                                               mask_logits_pred_b_sub_mask, relative_coords_b), dim=1)

                mask_feats_bi_final = (
                            self.conv_norm_relus_fuse_all_1(mask_feats_bi_new) + self.conv_norm_relus_fuse_all_2(
                        mask_feats_bi_new) + self.conv_norm_relus_fuse_all_3(
                        mask_feats_bi_new) + self.conv_norm_relus_fuse_all_4(mask_feats_bi_new)).unsqueeze(0)
...
def parse_dynamic_params(params, channels, weight_nums, bias_nums):
    ...
    a = sum(weight_nums)
    b = sum(bias_nums)
    c = sum(weight_nums) + sum(bias_nums)
    d = params.size(1)
...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers): #hidden_dim256
        super().__init__()
