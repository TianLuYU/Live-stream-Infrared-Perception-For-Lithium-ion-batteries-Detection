...
from .segmentation import (SeqFormer, PostProcessSegm,
                           dice_loss, dice_loss_my, sigmoid_focal_loss)
...
#初步看下来感觉是为了获得gt的incoherent mask
def get_incoherent_mask(input_masks, h_s, w_s):
    ...
    mask_uncertain = F.interpolate(mask_residue, (h_s, w_s), mode='bilinear')
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, gt_masks_uncertain, log=True):
    ...
    @torch.no_grad()
    def loss_cardinality(self, outputs, targets, indices, num_boxes, gt_masks_uncertain):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, gt_masks_uncertain):
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes, gt_masks_uncertain):
    ...
    def loss_masks_refine(self, outputs, targets, indices, num_boxes, gt_masks_uncertain):
        """Compute the losses related to the masks: the focal loss and the dice loss.
           targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """
        assert "pred_masks_refine" in outputs
        src_idx = self._get_src_permutation_idx(indices)
        tgt_idx = self._get_tgt_permutation_idx(indices)

        src_masks = outputs["pred_masks_refine"]
        bs = len(targets)
        # src_masks: bs x [1, num_inst, num_frames, H/4, W/4] or [bs, num_inst, num_frames, H/4, W/4]
        # src_masks: [num_insts, num_frames, H/4, M/4]
        # src_masks = src_masks[src_idx]
        ...
        losses = {
            "loss_mask_refine": sigmoid_focal_loss(src_masks, target_masks, num_boxes),
            "loss_dice_refine": dice_loss(src_masks, target_masks, num_boxes),
        }
        return losses

    def loss_incoherent(self, outputs, targets, indices, num_boxes, gt_masks_uncertain):
        pred_masks_uncertain = outputs["pred_masks_inc"]
        bs = len(targets)
        if type(pred_masks_uncertain) == list:
            pred_masks_uncertain = torch.cat(pred_masks_uncertain, dim=1)[0]
        if type(gt_masks_uncertain) == list:
            gt_masks_uncertain = torch.cat(gt_masks_uncertain, dim=1)[0]
        pred_masks_uncertain = torch.sigmoid(pred_masks_uncertain)
        # print(pred_masks_uncertain)
        losses = {
            "loss_incoherent": dice_loss_my(pred_masks_uncertain, gt_masks_uncertain, num_boxes) + F.binary_cross_entropy(pred_masks_uncertain.squeeze(1), gt_masks_uncertain, reduction="mean")
        }
        return losses
    
    def loss_incoherent_L1(self, outputs, targets, indices, num_boxes, gt_masks_uncertain):
        LIMIT = 30
        pred_masks_uncertain = outputs["pred_masks_inc"]
        bs = len(targets)
        if type(pred_masks_uncertain) == list:
        ...
        target_masks = target_masks.to(pred_masks_uncertain)
        target_masks = F.interpolate(target_masks, size=(30, 48), mode="bilinear", align_corners=False)
        pred_masks_uncertain = F.interpolate(pred_masks_uncertain, size=(30, 48), mode="bilinear", align_corners=False)
        pred_masks_uncertain = torch.sigmoid(pred_masks_uncertain)
        # torch.set_printoptions(profile="full")
  
        mask_uncertain_bool = (pred_masks_uncertain.detach() >= 0.125)
        uncertain_pos = torch.nonzero(mask_uncertain_bool.squeeze(1), as_tuple=True)

        target_uncertain = target_masks.permute(0, 2, 3, 1)[uncertain_pos]
        pred_uncertain = pred_masks_uncertain.permute(0, 2, 3, 1)[uncertain_pos]

        losses = {
            "loss_incoherent_L1": F.l1_loss(pred_uncertain, target_uncertain)
        }
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, gt_masks_uncertain, **kwargs):
        loss_map = {
            'labels': self.loss_labels,
            'cardinality': self.loss_cardinality,
            'boxes': self.loss_boxes,
            'masks': self.loss_masks,
            'masks_refine': self.loss_masks_refine,
            "masks_incoherent": self.loss_incoherent,
            "masks_incoherent_L1": self.loss_incoherent_L1
        }
        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        return loss_map[loss](outputs, targets, indices, num_boxes, gt_masks_uncertain, **kwargs)
    ...
    def forward(self, outputs, targets, indices_list, valid_ratios):
        ...
        for index, instance in enumerate(targets):
            if len(instance) == 0:
                continue
            gt_classes.append(instance)
            a = instance["masks"].size(1)
            b = instance["masks"].size(2)
            mask_uncertain = get_incoherent_mask(instance["masks"].unsqueeze(1), outputs["pred_masks_inc"][index].size(-2), outputs["pred_masks_inc"][index].size(-1))
            gt_masks_uncertain.append(mask_uncertain)
        ...
        for loss in self.losses:
            kwargs = {}
            losses.update(self.get_loss(loss, outputs, targets, indices_list[-1], num_boxes, gt_masks_uncertain, **kwargs))
            ...
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    if loss == 'labels':
                        ...
                        kwargs['log'] = False
                    l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_boxes, gt_masks_uncertain, **kwargs)
...
def build(args):    
    ...
    if args.masks:
        ...
        weight_dict["loss_dice_refine"] = args.dice_loss_coef
        weight_dict["loss_incoherent"] = 1.0
        weight_dict["loss_incoherent_L1"] = 1.0
    ...
    if args.masks:
        losses += ["masks"]
        losses += ["masks_refine"]
        losses += ["masks_incoherent"]
        losses += ["masks_incoherent_L1"]
